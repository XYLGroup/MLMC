...
class MLMC:
    ...
    def MCMC_sampling(self):
        ...
        elif self.conf.model == 'MLMC-motion':
        ...
        elif self.conf.model == 'MLMC-random-motion':
    ...
    def MC_warm_up(self):
        if (self.conf.model == 'MLMC' or self.conf.model == 'MLMC-motion' or self.conf.model == 'MLMC-random-motion'): # and iteration == 0
    ...
    def __init__(self, conf, lr, hr, device=torch.device('cuda')):
        ...
        if conf.model == 'MLMC' or conf.model == 'MLMC-motion' or conf.model == 'MLMC-random-motion':
            n_k = 200
            ...
            self.optimizer_kp = torch.optim.Adam([{'params': self.net_kp.parameters()}], lr=conf.MLMC_kp_lr) #  1e-4
        ...
        for self.iteration in tqdm.tqdm(range(self.conf.max_iters), ncols=60):
            ...
            if self.conf.model == 'MLMC':
                ...
                for i_p in range(self.conf.I_loop_x):
                    ...
                    if self.iteration <= 80:  #80 for MLMC
